...
export default function RecipesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1">{t.recipes.title}</h1>
        <p className="text-sm text-gray-500">{t.recipes.subtitle}</p>
      </div>

      <div className="space-y-4">
        {mockRecipes.map((recipe) => (
          <RecipeCard key={recipe.id} recipe={recipe} ingredientsLabel={t.recipes.ingredients} />
        ))}
      </div>
    </div>
  );
